...
public class POMDPLoader {
            ...
            if ((sLine.length() > 0) && (sLine.charAt(0) != '#')) {
                ...
                if (stLine.hasMoreTokens()) {
                    ...
                    if (sType.equals("T:")) {
                        ...
                        readTransition(lrInput, sValue, stLine); // 转移函数
                    } else if (sType.equals("O:")) {
                        sValue = stLine.nextToken();
                        readObservation(lrInput, sValue, stLine); // 观察函数
                    } else if (sType.equals("R:")) {
                        readReward(lrInput, stLine); // 收益函数
                    } else if (sType.equals("start:")) {
                        if (stLine.hasMoreTokens())
                            readStartState(lrInput, stLine); // 初始信念状态
                        else
                            readStartState(lrInput, null);
                    } else if (sType.equals("E:")) {
                        if (stLine.hasMoreTokens())
                            readTerminalStates(stLine); // 终止状态
                    } else if (sType.equals("OS:")) {
                        if (stLine.hasMoreTokens())
                            readObservationStates(stLine); // 观察值敏感状态
                    }
                }
            }
        }
    ...
    protected void verifyFunctions() {
        ...
        double dTr = 0.0, dSumTr = 0.0, dOb = 0.0, dSumOb = 0.0, dPr = 0.0, dSumPr = 0.0; // 分别是转移函数及和、观察函数及和、初始概率及和
        boolean bFixed = false;
        int cStates = m_pPOMDP.getStateCount();
        int cActions = m_pPOMDP.getActionCount();

        itNonZero = m_pPOMDP.getStartStates(); // 非0起始状态
        ...
        if (Math.abs(dSumPr - 1.0) > 0.0001) // 验证起始信念
        ...
        for (iAction = 0; iAction < cActions; iAction++) {
            for (iEndState = 0; iEndState < cStates; iEndState++) {
                ...
                if (Math.abs(dSumOb - 1.0) > 0.0001)
                    Logger.getInstance().logln("sum O( " + iAction + ", " + iEndState + ", * ) = " + dSumOb);
            }
        }
    }
    ...
    protected void readStartState(LineReader lrInput, StringTokenizer stLine) throws InvalidModelFileFormatException {
        ...
        try {
            if (stLine.countTokens() == 1) // 计算在生成异常之前可以调用此tokenizer的nextToken方法的次数
            {
                if (stLine.nextToken().equals("uniform")) // 均分概率
                {
            ...
            if (Math.abs(1.0 - dSumProbs) > 0.001) { // 验证起始概率之和为1
    ...
    protected void readHeader(LineReader lrInput) throws IOException {
        ...
        while (cVars < 5) { // 文件头一共5个信息，动作数、状态数、折扣因子、观察数和值
            sLine = "";
            while (sLine.equals("")) {
                sLine = lrInput.readLine();
                sLine = sLine.trim();
